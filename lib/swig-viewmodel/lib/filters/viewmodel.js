...
module.exports = {
    ...
    getJob: function(input, options) {
        ...
        function getContent() {
            if (options.preprocessContent && options.preprocessContent === true) {
                return stripHtmlTags(unescapeHtml(
                    escapist.unicode.unescape(greenhouse.get(input, 'content'))
                ));
            } else if (options.ignoreContent && options.ignoreContent === true) {
                return '';
            } else {
                return escapist.unicode.unescape(greenhouse.get(input, 'content'));
            }
        }
