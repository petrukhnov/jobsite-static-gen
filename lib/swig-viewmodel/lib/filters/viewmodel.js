...
module.exports = {
    ...
    getRstBlogpost: function(input, authorsViewmodel, options) {
        return getOldStyleBlogpost(input, authorsViewmodel, prismicRst, rst2html, options);
    },
...
function getOldStyleBlogpost(input, authorsViewmodel, dataExtractor, body2html, options) {
    options = options || {};
    var metaData = dataExtractor.getMetaData(input);
    var bodyTextContent  = dataExtractor.getBodyAsText(input);
    ...
    return {
        ...
        content: getContent(),
        tags: dataExtractor.getMetaDataTags(metaData, [])
    };
    ...
    function getContent() {
        return options.ignoreContent ? '' : body2html(bodyTextContent);
    }
}
