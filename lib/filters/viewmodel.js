...
module.exports = {
    ...
    getJob: function(input) {
        ...
        viewmodel.link = greenhouse.get(input, 'absolute_url');
        viewmodel.title = greenhouse.get(input, 'title');
        viewmodel.department = greenhouse.getDepartment(input);
        ...
        viewmodel.categories = _.uniq(categories);
        if (Math.random()>0.5) {
            viewmodel.locations.push('Dublin');
            if (Math.random()>0.5) {
                viewmodel.locations.push('Mönchengladbach');
            }
        }
